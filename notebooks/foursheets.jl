...
sqrt_with_phase(z, ϕ) = sqrt(z * cis(ϕ)) * cis(-ϕ / 2)
...
ω(ϵ) = sqrt_with_phase(ϵ, -π) + sqrt_with_phase(ϵ - 1, -π)
...
begin
    ω_II(ϵ) = -sqrt_with_phase(ϵ, -π) + sqrt_with_phase(ϵ - 1, -π)
    ω_III(ϵ) = sqrt_with_phase(ϵ, -π) - sqrt_with_phase(ϵ - 1, -π)
    ω_IV(ϵ) = -sqrt_with_phase(ϵ, -π) - sqrt_with_phase(ϵ - 1, -π)
end
...
function foursheets_omega_plane(grid_position)
    ...
    poly!(ax, Point2f.(reim.(ω_II.(semicirc(0, 50)))), color = :red)
    poly!(ax, Point2f.(reim.(ω_III.(semicirc(0, 50)))), color = :orange)
    ...
    sv = ["II", "III", "I", "IV"]
    xy = Point2f[(0.4, 0.4), (1.1, -1.1), (1.1, 1.1), (0.4, -0.4)]
    scatter!(ax, xy, markersize = 90, color = :white)
    text!(xy; text=sv, fontsize=30, align=(:center,:center))
end
